...
After cleaning the data, we wanted to look at leave status to know how many municipal employees were actively working, had stopped working at their jobs or were on leave.  We constructed a pie chart which shows the percentages of leave status for all municipal employees in 2022.  
...
Prior to computing the percentages and numbers of leave status employees for all 16 counties, we predicted that most of the municipal employees had an leave status of active which was in fact true that majority of the 594, 234 employees were actively working.  The remaining number of employees were either on leave or stopped working. 

When looking at the mean salary of municipal employees in New State York counties, we found it interesting that Albany had the highest mean base salary. We saw that there was a notable difference in the average base salary between New York County and Albany county. We assumed that the average base salary in New York county would be fairly high due to it being known as  a financial hub in New York State.  

Since Albany had the highest mean base salary, we decided to look at the job titles across all 16 counties in New York. We also expected that New York county would have the most number of job titles due its large population. When looking at the number of job titles in New York state counties, New York county did indeed have the highest number of job titles, with Queens county coming in second then Kings county and Bronx county. We found it interesting that  Albany county had the third lowest number of job titles meaning that the variability of jobs is low.  


We also looked at the top 5 job titles across Bronx, Kings, Queens, New York, and Albany counties.  The most prominent job titles were correction officer, assistant legislative representatives, teachers per session, and  correction officers.  It was interesting that Albany had the fewest number of job titles, their highest number being 4 and also having the highest mean base salary.  One could infer that the quality of jobs should be taken into account that a  higher quality job could mean a higher base salary.


We also looked at the median of the total other pay for municipal employees across all New York State counties in 2022 and found that New York county had the lowest median pay and Schoharie county had the highest median for total other pay which was  approximately more than 5,000 dollars for municipal employees. This was a result that we did not expect and we believed that New York county would also have the highest median. It is important to acknowledge that New York county had numerous outliers which could have been possible due to its large population. Other counties that had many outliers were Queens, Bronx, and Kings.   

In addition to looking at the mean base salary, we also chose to look at the median for the annual, daily, hourly, and prorated  base salary.  Albany had the highest annual median base salary with New York county coming in second. For the daily median base salary, Albany, Dutchess, and Greene county had the highest median base salary.  Nassau county had the highest hourly median base salary.  For prorated base mean salary , Ulster county had the highest followed by New York county.  We also determined which county has the highest median total overtime pay for municipal employees in New York state.  It was found that New York had the lowest median total overtime pay while Greene county had the highest median.  

In our statistical analysis, specifically the ANOVA test,  we claimed that there are significant  differences between the means of base salary for all New York State counties.  This hypothesis was proven to be true as the p value was zero allowing to reject the null hypothesis that there was no difference. We also predicted that there were significant differences in the mean base salary among all of the counties which led us to perform the Tukey test. In the Tukey test we saw that there were many significant differences in the mean base salary when comparing the counties among one another. Some significant differences were: New York and Albany, Kings and Bronx, New York and Bronx, Richmond and Kings, and Ulster and Kings.  Ultimately, we saw that most of the significant differences between NYC 5 boroughs and Albany as their p values were less than 0.05 which confirmed our claim. 
